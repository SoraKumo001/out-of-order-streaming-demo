...
Demo: [https://out-of-order-streaming-demo.vercel.app/](https://out-of-order-streaming-demo.vercel.app/)
...
- [vercel.json](./vercel.json)  
  `/` へのアクセスを Vercel Function の `/api/index` に転送します。
...
このコマンドは TypeScript の型チェックを行い、あわせて `dist/` を作成します。現在の構成では `dist/.keep` と `dist/style.css` が生成され、Vercel の Output Directory が `dist` の場合でも必要ファイルを配信できます。
...
Vercel は `api/index.ts` を Edge Function として扱い、`vercel.json` の rewrite によりトップページ `/` でアプリが表示されます。スタイルはビルド時に `public/style.css` から `dist/style.css` へコピーされ、`/style.css` として参照されます。
